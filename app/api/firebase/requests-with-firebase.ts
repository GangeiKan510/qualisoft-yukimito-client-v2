...
export const generateBlobWithFirebaseJwt = async (
  path: string,
  {
    body,
    headers,
  ...
  const response = await fetchWithFirebaseJwt(
  ...
  if (contentType && contentType.toLowerCase().includes("application/json")) {
    ...
    throw new Error(
      `Unexpected response data: ${JSON.stringify(responseData)}`,
    );
  } else {
...
export const getBlobWithFirebaseJwt = async (
  path: string,
  {
    params,
    headers = {},
  ...
  errorMessage = errorMessage || `Failed to get blob data from ${path}`;

  const response = await fetchWithFirebaseJwt(url, {
    method: "GET",
    headers: headers,
  });
